...
export async function saveBriefing(markdown, meta = {}, sources = []) {
  const key = todayKey();
  if (kvAvailable()) {
    await kvSetJSON(K(`brief:${key}`), { markdown: markdown.trim(), meta, sources, date: key }, { ex: DAY_TTL });
    return `KV brief:${key}`;
  }
  ensureDirs();
  // App-lesbar fasit (med kildemarkører) i data/
  atomicWrite(
    path.join(DATA_DIR, `brief-${key}.json`),
    JSON.stringify({ markdown: markdown.trim(), meta, sources, date: key }, null, 2)
  );
...
export async function readTodayBriefing() {
  ...
  if (kvAvailable()) {
    ...
    return j && j.markdown
      ? { markdown: j.markdown, meta: j.meta || {}, sources: j.sources || [], date: key }
      : null;
  }
  // Filmodus: JSON-fasiten (med markører + kilder) foretrekkes
  try {
    const j = JSON.parse(fs.readFileSync(path.join(DATA_DIR, `brief-${key}.json`), "utf8"));
    if (j && j.markdown) return { markdown: j.markdown, meta: j.meta || {}, sources: j.sources || [], date: key };
  ...
  return { markdown, meta, sources: [], date: key };
}
