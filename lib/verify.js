...
export function annotate({ markdown, sources = [], searchResults = [] }) {
  ...
  for (let line of lines) {
    ...
    if (bb && BB_DELTA[bb] !== undefined) {
      delta = BB_DELTA[bb];
      factors.push(`${bb.startsWith("HARD") ? "harde tall" : bb.startsWith("MYK") ? "mykt signal" : "nøytral"} ${delta >= 0 ? "+" : ""}${delta}`);
    } else {
      factors.push("ingen retningsfaktor ekstrahert (0)");
    }
    const matMult = MAT_MULT[mat] ?? 1.0;
    if (mat) factors.push(`vesentlighet ${mat.toLowerCase()} (x${matMult})`);
    let insiderDelta = 0;
    if (insider && INSIDER_DELTA[insider] !== undefined) {
      insiderDelta = INSIDER_DELTA[insider];
      factors.push(`innside${insider === "SALG" ? "salg" : "kjøp"} ${insiderDelta > 0 ? "+" : ""}${insiderDelta}`);
    }
    const confMult = CONF_MULT[level];
    factors.push(level === "ok" ? "bekreftet (x1.0)" : level === "single" ? "enkeltkilde (x0.6)" : "ubekreftet (x0.25)");
    let mult = confMult;
