...
11. Tall om innsidehandel, styringsrente/KPI/ledighet og kalenderdatoer kommer KUN fra STRUKTURERTE FAKTA-blokken (offisielle kilder hentet i kode). Søk aldri etter disse tallene, og hent dem ALDRI fra hukommelsen. Mangler blokken data om noe: si det, ikke gjett.`;
...
export function briefingUser({ dateLine, quotesBlock, focusItems, structuredBlock }) {
...
STRUKTURERTE FAKTA (autoritative tall hentet i kode fra SEC EDGAR, FRED, NewsWeb og resultatkalender - bruk KUN disse for innsidehandel, makrotall og datoer):
${structuredBlock || "Utilgjengelig i dag - ikke oppgi innsidetall eller makrotall uten sitert søkekilde."}
...
TL;DR-REGEL: Første linje i HVER ##-seksjon (etter ev. kurslinje) skal være \`TL;DR: <én setning med det aller viktigste>\`. Ingen tidstagg eller faktortagger på TL;DR-linjen.
...
// ---- delta-brief (intradag-oppdatering, fase 2) --------------------------------

export function deltaSystem() {
  return `Du er MORGENBRIEFs intradag-oppdatering. Du skriver KUN det som er NYTT siden morgenbriefen - aldri oppsummering av det som allerede står der.

${TONE_RULES}

Regler: bruk web_search for alt (maks 4 søk, siste timer prioriteres), siter hver påstand, tidstagg [I DAG HH:MM]-stil der klokkeslett er kjent, ellers [I DAG]. Bruk LIVE KURSER ordrett for priser. Ingenting vesentlig nytt: skriv nøyaktig "Ingenting vesentlig nytt siden i morges." Output: ren markdown, kun punktliste, maks 150 ord, ingen overskrifter.`;
}

export function deltaUser({ dateLine, morningBrief, quotesBlock }) {
  return `Nå er det ${dateLine}. Finn det viktigste som har skjedd ETTER at morgenbriefen under ble skrevet (markeder, fokus-selskapene, makro). Maks 5 punkter.

LIVE KURSER NÅ:
${quotesBlock}

=== MORGENBRIEFEN (skrevet tidligere i dag - IKKE gjenta innhold herfra) ===
${morningBrief}`;
}
