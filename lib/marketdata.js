...
export async function edgarFilings(ticker) {
  return cached(`edgar:${ticker}`, 6 * 3600, async () => {
    ...
    for (let i = 0; i < n && (filings.length < 8 || form4.length < 5); i++) {
      // SEC oppgir ofte "xslF345X05/dok.xml" (XSL-rendret HTML-visning).
      // Vi må ha RÅ XML-en for å lese transaksjonskoder og navn - strip prefikset.
      const rawDoc = String(r.primaryDocument[i]).replace(/^xslF[\w-]+\//i, "");
      const item = {
        form: r.form[i],
        date: r.filingDate[i],
        url: `${EDGAR_WWW()}/Archives/edgar/data/${Number(cik)}/${String(r.accessionNumber[i]).replaceAll("-", "")}/${rawDoc}`,
      };
    ...
    for (const f of form4) {
      try {
        ...
        // Navn: rptOwnerName er standard; reportingOwnerId-varianter dekkes også.
        f.owner = ((xml.match(/<rptOwnerName>\s*([^<]+?)\s*<\/rptOwnerName>/i) || [])[1] || "")
          .replace(/\s+/g, " ").trim();
        f.shares = Number((xml.match(/<transactionShares>\s*<value>\s*([\d.]+)/i) || [])[1] || 0);
      } catch {
        f.side = "UKJENT";
      }
    }
...
export async function fredReleaseDates() {
  ...
  return cached("freddates", 12 * 3600, async () => {
    ...
    for (const [id, label] of releases) {
      try {
        // sort_order=desc + limit er PÅKREVD her: default gir eldste datoer først
        // (1990-tallet), som gjorde at fremtidige slipp aldri kom med.
        const j = await get(`${FRED_BASE()}/fred/release/dates?release_id=${id}&api_key=${key}&file_type=json&include_release_dates_with_no_data=true&sort_order=desc&limit=30`);
...
export function buildCountdown(events, todayStr) {
  // «i dag» må regnes i norsk tid, ikke UTC - ellers er nedtellingen én dag
  // feil mellom midnatt norsk tid og midnatt UTC.
  const osloToday = todayStr || new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Oslo" }).format(new Date());
  const today = new Date(osloToday + "T00:00:00Z");
  const seen = new Set();
  return (events || [])
    .filter((e) => e && e.date)
